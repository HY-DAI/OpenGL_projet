...
struct MeshProgram
{
    ...
    GLint locationMVPMatrix;
    GLint locationMVMatrix;
    GLint locationNormalMatrix;
    // positions des unites de texture
    GLint locationColorTexture;
    GLint locationBonusTexture;
    // positions des param de lumière
    GLuint locationAmbient;
    GLint locationKd;             // coefficient de reflection diffuse de l'objet
    GLint locationKs;             // coefficient de reflection glossy de l'objet
    GLint locationShininess;      // exposant de brillance permettant de controller la taille de la "tache" de brillance glossy
    GLint locationLightDir_vs;    // direction incidente
    GLint locationLightIntensity; // intensite de la lumière incidente
    GLuint locationLightPos_vs1;
    GLuint locationLightPos_vs2;
    GLuint locationLightIntensity1;
    GLuint locationLightIntensity2;

    MeshProgram(const FilePath &applicationPath) : m_Program(loadProgram(applicationPath.dirPath() + "shaders/3D.vs.glsl",
                                                             applicationPath.dirPath() + "shaders/resume.fs.glsl"))
    {
        // positions des Matrices de transformation de repere
        ...
        locationBonusTexture = glGetUniformLocation(m_Program.getGLId(), "uBonusTexture");
        // positions des param de lumière
        locationAmbient = glGetUniformLocation(m_Program.getGLId(), "uAmbiantLight");
        locationKd = glGetUniformLocation(m_Program.getGLId(), "uKd");
        locationKs = glGetUniformLocation(m_Program.getGLId(), "uKs");
        locationShininess = glGetUniformLocation(m_Program.getGLId(), "uShininess");

        locationLightDir_vs = glGetUniformLocation(m_Program.getGLId(), "uLightDir_vs");
        locationLightIntensity = glGetUniformLocation(m_Program.getGLId(), "uLightIntensity");
        locationLightPos_vs1 = glGetUniformLocation(m_Program.getGLId(), "uLightPos_vs1");
        locationLightPos_vs2 = glGetUniformLocation(m_Program.getGLId(), "uLightPos_vs2");
        locationLightIntensity1 = glGetUniformLocation(m_Program.getGLId(), "uLightIntensity1");
        locationLightIntensity2 = glGetUniformLocation(m_Program.getGLId(), "uLightIntensity2");
    }
};
...
int main(int argc, char **argv)
{
    ...
    while (!done)
    {
        ...
        glUniformMatrix4fv(meshProgram.locationMVMatrix, 1, GL_FALSE, glm::value_ptr(MVMatrix));
        glUniformMatrix4fv(meshProgram.locationNormalMatrix, 1, GL_FALSE, glm::value_ptr(NormalMatrix));
        glUniformMatrix4fv(meshProgram.locationMVPMatrix, 1, GL_FALSE, glm::value_ptr(ProjMatrix * MVMatrix));

        // Light Characteristics of the shape
        glUniform1f(meshProgram.locationShininess, uShininess);
        glUniform3fv(meshProgram.locationKd, 1, glm::value_ptr(uKd));
        glUniform3fv(meshProgram.locationKs, 1, glm::value_ptr(uKs));

        // Ambient light
        glUniform3fv(meshProgram.locationAmbient, 1, glm::value_ptr(uAmbiantLight));        
        
        // Directional light
        glm::vec3 lightDir_vs = glm::vec3(MatView * glm::vec4(uLightDir_vs, 0));
        lightDir_vs = static_cast<float>(dir_light) * lightDir_vs;
        glUniform3fv(meshProgram.locationLightDir_vs, 1, glm::value_ptr(lightDir_vs));
        glUniform3fv(meshProgram.locationLightIntensity, 1, glm::value_ptr(uLightIntensity));

        // Point Lights
        glm::vec3 lightPos_vs2 = glm::vec3(MatView * glm::vec4(uLightPos_vs2, 1));
        glUniform3fv(meshProgram.locationLightPos_vs1, 1, glm::value_ptr(uLightPos_vs1));
        glUniform3fv(meshProgram.locationLightPos_vs2, 1, glm::value_ptr(lightPos_vs2));      
        glUniform3fv(meshProgram.locationLightIntensity1, 1, glm::value_ptr(10.f * uLightIntensity));
        glUniform3fv(meshProgram.locationLightIntensity2, 1, glm::value_ptr(10.f * uLightIntensity));
        ...
        for (int i = 0; i < material_positions.size() - 1; i++)
        {
            ...
            glUniform1i(meshProgram.locationColorTexture, 0);
            glActiveTexture(GL_TEXTURE0);
        ...
        meshProgram.m_Program.use();
            ...
            glUniformMatrix4fv(meshProgram.locationMVMatrix, 1, GL_FALSE, glm::value_ptr(MVMatrix2));
            glUniformMatrix4fv(meshProgram.locationNormalMatrix, 1, GL_FALSE, glm::value_ptr(NormalMatrix2));
            glUniformMatrix4fv(meshProgram.locationMVPMatrix, 1, GL_FALSE, glm::value_ptr(ProjMatrix * MVMatrix2));
